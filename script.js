...
function resize() {
  ...
  document.documentElement.style.setProperty(
    "--scrollbar-width",
    Math.ceil(container.getBoundingClientRect().width - container.scrollWidth) +
      "px"
  );
}
...
fixit.onpaste = (e) => {
  try {
    ...
    if (!gdoc) return;
    e.preventDefault();
    const data = JSON.parse(JSON.parse(gdoc).data);
    let symbols = data.resolved.dsl_styleslices.find(
      (e) => e.stsl_type == "equation_function"
    ).stsl_styles;
    ...
    resize();
    stats.beacon("converted");
  } catch (e) {
    stats.beacon("error");
    Swal.fire({
      title: "Error",
      html: `There was an error processing what you pasted. To help get the issue fixed, please paste the content causing the error into a new document, enable link sharing, and send the link to <a href="mailto:ffixer@easrng.net">ffixer@easrng.net</a>. Thanks!`,
    });
  }
};
info.onclick = () =>
